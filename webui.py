...
def generate_clicked(task: worker.AsyncTask):
    ...
    try:
        while not finished:
            ...
            # ── Deadlock watchdog ─────────────────────────────────────────────
            # If the worker has finished processing AND has no more queued
            # yields, it means it completed (or crashed) without ever sending
            # 'finish'.  Force a finish so the UI doesn't loop forever.
            if not task.processing and len(task.yields) == 0 and not finished:
                # Only trigger watchdog after the worker has had a chance to
                # start (processing goes True→False, or it crashed before True)
                import time as _time
                if _time.perf_counter() - execution_start_time > 2.0:
                    print('[Fooocus] Watchdog: task appears dead — forcing finish')
            ...
            if len(task.yields) > 0:
                ...
                if flag == 'preview':
                    # Skip this preview ONLY when there are 2+ more previews
                    # queued (not just 1). Original code skipped even if only 1
                    # was queued which silently dropped all previews on fast GPUs.
                    pending_previews = sum(1 for f, _ in task.yields if f == 'preview')
                    if pending_previews >= 2:
                        continue
        ...
        # User navigated away or refreshed — stop the worker gracefully
        task.last_stop = 'stop'
...
with shared.gradio_root:
    ...
    no_model_warning_row = gr.Row(visible=len(modules.config.model_filenames) == 0, elem_id='no_model_warning_row')
    with no_model_warning_row:
        gr.HTML(
            f'<div style="background-color: #ffe6e6; border: 1px solid #ffb3b3; color: #b30000; padding: 15px; border-radius: 8px; margin: 10px 0; font-weight: bold; font-size: 1.1em; width: 100%;">'
            f'⚠️ <strong>No model files found in your checkpoints folder!</strong><br/>'
            f'Please place your SDXL model checkpoint files (e.g., in <code>.safetensors</code> format) in the <code>models/checkpoints</code> folder, '
            f'then restart the application or click "Refresh All Files" in the Models tab.'
            f'</div>'
        )
    ...
    with gr.Row():
        with gr.Column(scale=2):
            ...
            with gr.Row():
                with gr.Column(scale=1, min_width=50, elem_id='wildcard_col'):
                    def get_wildcard_tags():
                        try:
                            ...
                            return [f'__{n}__' for n in names]
                        except Exception:
                            return []

                    wildcard_choices = get_wildcard_tags()

                    with gr.Accordion(label="📋", open=False, elem_id="wildcard_accordion"):
                        wildcard_checkboxes = gr.CheckboxGroup(
                ...
                with gr.Column(scale=16):
                    ...
                    negative_prompt = gr.Textbox(show_label=False, placeholder="Type negative prompt here.",
                                                 lines=3, elem_id='negative_prompt',
                ...
                def on_wildcard_checkbox_change(selected_tags, current_prompt):
                    current_prompt = current_prompt or ""
                    import re
                    existing_tags = re.findall(r'__[\w-]+__', current_prompt)

                    new_prompt = current_prompt
                    for tag in selected_tags:
                        if tag not in existing_tags:
                            tag_str = f"{tag}, "
                            if new_prompt and not new_prompt.rstrip().endswith((',', ' ')):
                                new_prompt = new_prompt.rstrip() + ', ' + tag_str
                            elif new_prompt:
                                new_prompt = new_prompt.rstrip() + ' ' + tag_str
                            else:
                                new_prompt = tag_str

                    for tag in wildcard_choices:
                        if tag not in selected_tags and tag in existing_tags:
                            pattern = re.escape(tag) + r'\s*,\s*|' + re.escape(tag)
                            new_prompt = re.sub(pattern, '', new_prompt).strip()

                    return new_prompt

                def sync_wildcards_from_prompt(current_prompt):
                    current_prompt = current_prompt or ""
                    import re
                    found_tags = re.findall(r'__[\w-]+__', current_prompt)
                    selected = [t for t in found_tags if t in wildcard_choices]
                    return gr.update(value=selected)

                wildcard_checkboxes.change(
                    fn=on_wildcard_checkbox_change,
                    inputs=[wildcard_checkboxes, prompt],
                    outputs=[prompt],
                    queue=False, show_progress=False
                )

                prompt.change(
                    fn=sync_wildcards_from_prompt,
                    inputs=[prompt],
                    outputs=[wildcard_checkboxes],
                    queue=False, show_progress=False
                )
                        ...
                        with gr.Row():
                            with gr.Column():
                                def swap_to_gallery(images):
                                    ...
                                    return gr.update(value=pil_images, visible=True), gr.update(visible=False)
            ...
            with gr.Tab(label='Settings'):
                ...
                with gr.Accordion(label='Aspect Ratios', open=False, elem_id='aspect_ratios_accordion') as aspect_ratios_accordion:
                    ...
                    gr.HTML('<div style="margin-top:10px;font-size:0.85em;color:#888;font-weight:600;">'
                            '📏 Custom Size (multiples of 64 recommended for best quality)</div>')
                    with gr.Row(elem_id='custom_size_row'):
                        custom_width = gr.Number(
                            label='Width', value=1152, minimum=64, maximum=8192,
                            step=64, precision=0, elem_id='custom_width',
                            scale=2
                        )
                        custom_height = gr.Number(
                            label='Height', value=896, minimum=64, maximum=8192,
                            step=64, precision=0, elem_id='custom_height',
                            scale=2
                        )
                        apply_custom_size_btn = gr.Button(
                            value='✓ Apply', variant='primary',
                            elem_id='apply_custom_size', scale=1
                        )
                    custom_size_status = gr.HTML(value='', elem_id='custom_size_status')

                    def apply_custom_size(w, h):
                        w, h = int(w), int(h)
                        # Round to nearest 64 for best SDXL results
                        w_snapped = max(64, round(w / 64) * 64)
                        h_snapped = max(64, round(h / 64) * 64)
                        warn = ''
                        if w != w_snapped or h != h_snapped:
                            warn = (f' <span style="color:orange;">⚠️ Snapped to {w_snapped}×{h_snapped} '
                                    f'(nearest 64-multiple)</span>')
                    ...
                    gr.HTML('<div style="font-size:0.78em;color:#999;margin-top:4px;">'
                            'Custom size overrides the aspect ratio selection above. '
                            'Set Width and Height to <b>-1</b> in the Advanced tab to use the preset ratio again.</div>')
        ...
        def generate_model_comparison_clicked(model_list, *ctrls_args):
            ...
            # Dedicated output directory for this comparison run
            ...
            print(f"[Model Comparison] Created comparison folder: {comp_folder}")
            print(f"[Model Comparison] Running comparison for {len(model_list)} models...")

            old_path_outputs = modules.config.path_outputs
            try:
