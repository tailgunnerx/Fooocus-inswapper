...
_instantid_path = os.path.join(_repo_root, 'InstantID')
if _instantid_path not in sys.path:
    sys.path.insert(0, _instantid_path)
...
def resize_img(input_image, max_side=1280, min_side=1024, size=None, pad_to_max_side=False, mode=Image.Resampling.LANCZOS, base_pixel_number=64):
...
def enhance_face_region(width, height, face_info):
  ...
  return control_mask  # Bug fix: was missing return, causing None to be passed to pipe()
