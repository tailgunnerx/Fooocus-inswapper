...
_photomaker_path = os.path.join(_repo_root, 'photomaker')
if _photomaker_path not in sys.path:
    sys.path.insert(0, _photomaker_path)
...
def generate_photomaker(prompt, input_id_images, negative_prompt, steps, seed, width, height, guidance_scale, loras, sampler_name, scheduler_name, async_task):
    ...
    # Convert images to bytes for processing (tobytes() call fixed — was missing parentheses)
    bytes_images = []
...
def get_sampler(sampler_name, scheduler_name):
    ...
    if (sampler_name) == "dpmpp_2m_sde_gpu":
        ...
        # timestep_spacing must be a string ("leading", "trailing", "linspace"), not a bool
        # rescale_betas_zero_snr removed — only valid in diffusers>=0.28 and causes errors on 0.26.x
        return DDIMScheduler(timestep_spacing="leading")